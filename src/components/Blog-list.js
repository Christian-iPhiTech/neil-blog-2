...
const BlogList = (props) => {
    ...
    return ( 
        <div className="blog-list flex flex-row flex-wrap justify-start gap-12">
            {blogs.map((blogs) => (
                    <div className="blog-items box-shadow-1 p-[30px] w-[48%] flex flex-col justify-between gap-3" key={blogs.id}>
                        ...
                        <div className="flex flex-row flex-wrap justify-end gap-7">
                            ...
                            {isDeleting.id !== blogs.id && <button className="rounded-lg p-3 bg-red-500 text-white font-bold" onClick={() => {handleDelete(blogs.id)}}>Delete</button>}
                            {isDeleting.loading && isDeleting.id === blogs.id &&<button className="rounded-lg p-3 bg-red-500 text-white font-bold" disabled>Deleting Blogs...</button>}
                        </div>
                    </div>
            ))}
        </div>
